...
namespace TravelingApp
{
    public class SlidingTabStrip : LinearLayout
    {
        ...
        protected override void OnDraw(Canvas canvas)
        {
            ...
            SlidingTabScrollView.TabColorizer tabColorizer = mCustomTabColorizer != null ? mCustomTabColorizer : mDefaultTabColorizer; //see if I set color
        ...
        private int blendColor(int color1, int color2, float ratio)
        {
            ...
            float r = (Color.GetRedComponent(color2) * ratio) + (Color.GetRedComponent(color1) * inverseRatio);
            float g = (Color.GetGreenComponent(color2) * ratio) + (Color.GetGreenComponent(color1) * inverseRatio);
            float b = (Color.GetBlueComponent(color2) * ratio) + (Color.GetBlueComponent(color1) * inverseRatio);
